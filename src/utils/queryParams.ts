...
export const useQueryParam = (key: string, value?: string) => {
  ...
  return [param, setter] as const;
};

export const useQueryParamAsNumber = (key: string, value?: number) => {
  // Relies on useState to trigger component rerenders on calls to setParam.
  const [param, setParam] = useState(handleParam(key, value));
  // override allows changing options for individual setQueryParam calls
  const setter = (newValue: number) => setParam(handleParam(key, newValue));
  return [parseInt(param, 10), setter] as const;
};
